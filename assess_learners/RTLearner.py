...
class RTLearner(object):
    ...
    def build_tree(self, data):
        """
        Recursively build the Random Tree to split with a specified leaf size.

        :param data: Maximum number of samples to form a leaf.
        :type data: numpy.ndarray
        :return: Random Tree node
        :rtype: numpy.ndarray
        """
        data_y = data[:, -1]
        ...
        # if all data.y same: return ['leaf', data.y, NA, NA]
            ...
            # Determine the random feature to split
            if data.ndim == 1: random_feat = np.random.randint(0, data.shape[0] - 1)
            else: random_feat = np.random.randint(0, data.shape[1] - 1)
            split_val = np.median(data[:, random_feat])

            # Split data based on the median value of the best feature
            left_data = data[data[:, random_feat] <= split_val]
            right_data = data[data[:, random_feat] > split_val]
            ...
            root = np.array([random_feat, split_val, 1, left_tree.shape[0] + 1])

            # Recursion edge case to prevent skipping right_tree leafs when left_tree.shape == (4,):
            if left_tree.ndim == 1: root = np.array([random_feat, split_val, 1, 2])
